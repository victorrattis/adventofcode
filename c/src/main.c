...
struct Point findStartPoint(char grid[140][140], int line, int column) {
    struct Point p;
...
struct Point nextPosition(struct Point previous, char value, struct Point current) {
    switch (value){
        case '-':
            if (current.y - 1 == previous.y) return createPoint(current.x, current.y + 1);
            else return createPoint(current.x, current.y - 1);
            break;
        case '|':
            if (current.x - 1 == previous.x) return createPoint(current.x + 1, current.y);
            else  return createPoint(current.x - 1, current.y);
            break;
        case 'L':
            if (current.y == previous.y) return createPoint(current.x, current.y + 1);
            else  return createPoint(current.x - 1, current.y);
            break;
        case 'J':
            if (current.y == previous.y) return createPoint(current.x, current.y - 1);
...
int checkConnectionWithStartTile(struct Point start, char value, struct Point current) {
    switch (value){
        case '-':
            return start.y != current.y && start.x == current.x;
            break;
        case '|':
        ...
        case 'J':
            return (start.y < current.y && start.x == current.x) ||
                (start.y == current.y && current.x > start.x);
            break;
        case '7':
            return (start.y < current.y && start.x == current.x) ||
                (start.y == current.y && current.x < start.x);
...
struct Point findPossibleFirstPath(struct Point start) {
    if (start.y < column - 1 && checkConnectionWithStartTile(
        start, grid[start.x][start.y + 1], createPoint(start.x, start.y + 1)
    )) {
        return createPoint(start.x, start.y + 1);
    } else if (start.y > 0 && checkConnectionWithStartTile(
        start, grid[start.x][start.y - 1], createPoint(start.x, start.y - 1)
    )) {
        return createPoint(start.x, start.y - 1);
    } else if (start.x > 0 && checkConnectionWithStartTile(
        start, grid[start.x - 1][start.y], createPoint(start.x - 1, start.y)
    )) {
        return createPoint(start.x - 1, start.y);
    } else if (start.x < line - 1 && checkConnectionWithStartTile(
        start, grid[start.x + 1][start.y], createPoint(start.x + 1, start.y)
    )) {
        return createPoint(start.x + 1, start.y);
    } else {
...
struct Answer calculateQuestion10() {
    struct Point start = findStartPoint(grid, line, column);
    struct Point current = findPossibleFirstPath(start);
    struct Point previous = start;
    struct Point next;
    ...
    while(!(current.x == start.x && current.y == start.y)) {
        next = nextPosition(previous, grid[current.x][current.y], current);
        previous = current;
        current = next;
        steps += 1;
    ...
    answer.part1 = steps / 2;
    answer.part2 = (abs(area) / 2) - (steps / 2) + 1;
...
int main() {
    ...
    CHECK_TIME(result = calculateQuestion10());
